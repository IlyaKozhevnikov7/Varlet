...
{
	InputInternal::UpdateMousePosition(xpos, ypos);
}
...
static void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	const KeyState state = action == GLFW_PRESS ? KeyState::Pressed : KeyState::Released;

#define KEY_CASE(NativeKey, Key) case NativeKey : InputInternal::AddKey(Key, state); break;
...
static void MouseButtonCallback(GLFWwindow* window, int button, int action, int mods)
{
	...
	switch (button)
	{
	case GLFW_MOUSE_BUTTON_LEFT:	InputInternal::AddMouseButton(MouseCode::Left, state);		break;
	case GLFW_MOUSE_BUTTON_RIGHT:	InputInternal::AddMouseButton(MouseCode::Right, state);		break;
	case GLFW_MOUSE_BUTTON_MIDDLE:	InputInternal::AddMouseButton(MouseCode::Middle, state);	break;
	}
}
